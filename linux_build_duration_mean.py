...
def Main():
	...
	mongo = collection.find({"region": "Fremont" }).sort("date_str",pymongo.ASCENDING)
	    ...
	    obj['Fremont'].append(index['durations_and_overhead']['linux_build']['duration_mean'])          

	mongo = collection.find({"region": "IDC" }).sort("date_str",pymongo.ASCENDING)
        for index in mongo:
	    obj['IDC'].append(index['durations_and_overhead']['linux_build']['duration_mean'])    

	mongo = collection.find({"region": "vCommander+Fremont" }).sort("date_str",pymongo.ASCENDING)
        for index in mongo:
	    obj['vCommander+Fremont'].append(index['durations_and_overhead']['linux_build']['duration_mean'])  

	mongo = collection.find({"region": "vCommander+IDC" }).sort("date_str",pymongo.ASCENDING)
        for index in mongo:
	    obj['vCommander+IDC'].append(index['durations_and_overhead']['linux_build']['duration_mean']) 

        myfile = open( '/var/www/html/fierybuild/csv/linux_build_duration_mean.csv', 'w')
