...
def Main():
	...
	mongo = collection.find({"region": "Fremont" }).sort("date_str",pymongo.ASCENDING)
	    ...
	    day = int(date_str[6:])
            ...
            weekly_sum += int(index['durations_and_overhead']['windows_install']['duration_median'])
            weekly_num.append( int(index['durations_and_overhead']['windows_install']['duration_median']))
	...
	mongo = collection.find({"region": "vCommander+IDC" }).sort("date_str",pymongo.ASCENDING)
        ...
        myfile = open( '/var/www/html/fierybuild/csv/weekly_windows_install_duration_median.csv', 'w')
