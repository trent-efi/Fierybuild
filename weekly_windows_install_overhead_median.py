...
def Main():
	...
	mongo = collection.find({"region": "Fremont" }).sort("date_str",pymongo.ASCENDING)
	    ...
	    day = int(date_str[6:])
            foo_date = date(year, month, day)
            try:
                weekly_sum += int(index['durations_and_overhead']['windows_install']['overhead_median'])
                weekly_num.append( int(index['durations_and_overhead']['windows_install']['overhead_median']))
	    except:
	        break
	...
	mongo = collection.find({"region": "vCommander+IDC" }).sort("date_str",pymongo.ASCENDING)
        ...
        myfile = open( '/var/www/html/fierybuild/csv/weekly_windows_install_overhead_median.csv', 'w')
