...
def Main():
	...
	mongo = collection.find({"region": "Fremont" }).sort("date_str",pymongo.ASCENDING)
	    ...
	    day = int(date_str[6:])
            foo_date = date(year, month, day)
            try:
                weekly_sum += int(index['daily_stats']['builds']['linux_wait'])
                weekly_num.append( int(index['daily_stats']['builds']['linux_wait']))
	    except:
	        break
	...
	mongo = collection.find({"region": "vCommander+IDC" }).sort("date_str",pymongo.ASCENDING)
        ...
        myfile = open( '/var/www/html/fierybuild/csv/weekly_linux_build_wait_time.csv', 'w')
