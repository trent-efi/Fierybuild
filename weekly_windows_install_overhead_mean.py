...
def Main():
	...
	mongo = collection.find({"region": "Fremont" }).sort("date_str",pymongo.ASCENDING)
	    ...
	    day = int(date_str[6:])
            ...
            weekly_sum += int(index['durations_and_overhead']['windows_install']['overhead_mean'])
            weekly_num.append( int(index['durations_and_overhead']['windows_install']['overhead_mean']))
	...
	mongo = collection.find({"region": "IDC" }).sort("date_str",pymongo.ASCENDING)
		...
		length = len(weekly_num)
                obj['IDC'].append(weekly_sum/length)
	...
	mongo = collection.find({"region": "vCommander+Fremont" }).sort("date_str",pymongo.ASCENDING)
		...
		length = len(weekly_num)
                obj['vCommander+Fremont'].append(weekly_sum/length)
	...
	mongo = collection.find({"region": "vCommander+IDC" }).sort("date_str",pymongo.ASCENDING)
		...
		length = len(weekly_num)
                obj['vCommander+IDC'].append(weekly_sum/length)
        ...
        myfile = open( '/var/www/html/fierybuild/csv/weekly_windows_install_overhead_mean.csv', 'w')
